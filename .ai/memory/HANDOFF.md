...
**Last updated: 2026-09-24 (phone gestures off across every toy via game-screen.js — `271a857`).**
...
*"I wouldn't want a user practicing forever here then going there and solving it really fast… same
for all feeders."* Global DECISIONS 2026-09-12. Audited all seven feeders and fixed the leaks:
- **Spelling Blocks** `data.js` had shipped all 91 dailies WITH answers (the hint gave daily solutions):
  now practice-only, GENERATED by `scripts/build-feeder-data.mjs` in the spelling-blocks repo.
- **Tiny Across** `puzzles.js`: rebuilt pool shares no answer+clue with any daily (export from its repo).
- **Brick Smasher** `script.js`: practice walls armour the bottom-row brick in column 4 (`MARK_COL`),
  mirrored from `practiceLayout()` in its repo; keep them in step.
...
⚠ **Tiny Across share image is OWNER ART, not og-gen** (`4baa226`): `assets/og/tiny-across.jpg`, a 1200x628
JPEG cropped from `assets/featured/_sources/tiny-across.png` (30px off the top, 36 off the bottom), which is
also the featured art. ⚠ **Keep it a JPEG under ~300KB** (WhatsApp drops heavier previews; a PNG of this
render is 2MB). The og-gen entry is commented UNUSED. The scene regenerates in the tiny-across repo
(`node scripts/gen-assets.mjs --opt <out.png>`). ⚠ **It shows daily No. 1's answers**, fine only because No. 1
is past: never put a current or future daily's answers in any image.
