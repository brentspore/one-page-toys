...
**Last updated:** 2026-07-08 · **Latest pushed site commit:** `main 35dd2de` · **71 toys live** at onepagetoys.com.

- Latest shipped round (`35dd2de`, 2026-07-08): **NEW 071 Rain Stick** (`toys/rain-stick/`, category `audio`) — a tilt-and-pour rain stick: a glass tube of ~460 small glass beads bounce down through a dense field of ~123 jittered thorns onto the low end; warm desert-dusk scene, turned-wood caps. Tip via **drag** (works everywhere), **device-motion tilt** (mobile, behind the iOS `DeviceMotionEvent.requestPermission` tap — Tilt btn hidden where no motion), or **Flip** (the full downpour); quick tap = shake. **Physics = a sleep system**: a bead that is SUPPORTED (a contact whose push opposes gravity) and slow FREEZES into a solid, silent, immovable collider — so the shower fades to true silence and the pile never jitters under the column's weight (velocity-based + `separate()` treats frozen beads as static walls; wake-all on tilt change). **Audio = granular**: a filtered-noise bed that follows the collision rate + velocity-scaled woody tick grains struck from actual bead-on-thorn hits, pitch-spread by bead size (`pm=3/r`) and panned by real on-screen position so the cascade sweeps as it falls. Perf: cached bead sprites, Int32Array linked-list spatial hash, baked backdrop → ~15ms/frame headless (60fps real). Cascade ~5.5s then settles. `script.js?v=4`, hub `styles.css?v=80`/`main.js?v=69`. **Audio timbre + gyro mapping (`gyroSign`) still need the owner's real device** — tuned/built by first principles, headless can't hear or feel tilt.
- Prior round (`b97eedd`, 2026-07-08): **NEW 070 Marble Machine** (`toys/marble-machine/`, category `audio`) — programmable marble music machine: 8-lane × 16-step peg barrel → glass marbles fall onto tuned vibraphone bars (impact = the note) → bucket-lift returns them to the reservoir. Vibraphone voice on the Tongue-Drum bus (motor tremolo LFO, convolver hall, tempo-synced echo), Major/Minor/Akebono, pattern/bpm/scale in localStorage. **Presentation is pseudo-3D**: perspective projection (`proj`/`toPlane` in script.js), cabinet = deep shadowbox (back panel z=-55, face z=+25) with interior reveals/side walls/parallax, idle ±10° sway (16s cycle; pauses while editing), drag-the-wood manual swivel (±17°), taps inverse-projected, reduced-motion = static angle. Rendering = column-strip warp of two flat textures (yaw-only rotation keeps verticals vertical, so per-column strips are exact) + individually projected dynamics.
...
**Next step:** Owner to real-device test Rain Stick audio timbre + gyro tilt (build is live either way); Marble Machine audio play-test also still open. Backlog: Alto's-style sandboarder (marked `next`), LinkedIn-Zip-style daily path puzzle (queued). See `.ai/memory/BACKLOG.md`.
