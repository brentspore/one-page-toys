...
**Notes:** Owner directive (2026-07-13): play and share, "not everything this site does." KEEP: the daily 6x6 board, drag-path drawing (fat rounded ribbon, drag back to unwind, instant win check), solve timer, share. CUT: stats modal, streaks, unlimited mode, how-to modal (hint line instead; per-day best in localStorage at most). **Same puzzle as the site** (recommended default — it's what makes the feeder feed): port the validated generator from the-trail-game `src/lib/game/generator.ts` (seed `trail:YYYY-MM-DD` from the player's LOCAL date, cyrb53 → mulberry32, serpentine + 4000 backbites, weekday dots/walls table) so "today's trail" is identical everywhere; match the share line format ("The Trail Game #N in M:SS") and the site's `LAUNCH_DATE` numbering so share texts are interchangeable. The generator is frozen and validated over there; if its weekday table ever changes, update this port in the same round (the-trail-game's BACKLOG carries the mirror item). **Feeder link needs an owner call at build time:** the 2026-06-11 no-sales-surfaces decision limits sister-site links to passive footer text with no tracking params, but this toy's purpose is a visible "full game at thetrailgame.com" link with a measurable source tag — scope placement and tagging with the owner then, as a documented per-toy exception if he confirms. Naming: carrying the real The Trail Game name is deliberate brand-building now that it's our property (the old invent-a-name legal note is obsolete); keep "zip"/"linkedin"/"daily"/"one line"/"path puzzle"/"trail" in NL search keywords only, and mind SEO overlap with the real site (toy canonical stays onepagetoys.com). **Audio quality bar applies:** pencil-slide tick per cell, rising pentatonic ping per dot reached, warm win chime (bus → reverb, iOS unlock, Sound toggle). Art: warm paper-board world per the original concept. Full add-a-toy pipeline (registry prepend `game`, sitemap, NL phrases, card rule + `:not()`, og-gen, real rendered card/OG mid-solve, hub cache-bust, headless verify incl. 375px + a scripted solve of the daily board — the-trail-game's `reference.md` documents the Playwright drive incl. the touch rAF-pacing gotcha, temp hooks removed).
---
...
---### Ticket system Phase 2 — store page, trophy shelf, gag items, share loop

**Why it matters:** Phase 1 (2026-07-21) ships the silent earning engine — every toy quietly accrues tickets, but there is nowhere to spend them and no UI. Phase 2 is what turns an invisible counter into an actual reward loop: a store, a shelf to show off, and a share hook. Until it lands, Phase 1 is banking history with no payoff.

**When to revisit:** Once Phase 1 has run silently long enough to sanity-check the earn rates against real play (owner testing via `?tickets=1`), and the balance numbers feel right. Tune the economy BEFORE building the store — item prices depend on realistic earn rates.

**Notes:** Owner picked **vanity collectibles + gag/novelty items only** (2026-07-21) — deliberately NO per-toy cosmetic unlocks, so nothing in the 87 toys gets edited and Phase 3 does not exist. Build: (1) `store/index.html` in the Geist hub chrome (light/dark, copy the `tools/` family shell), (2) `store-items.json` catalog (id/name/price/type/description/emoji-or-svg), (3) trophy shelf rendering owned items, (4) gag items executed from `assets/tickets.js` itself since it already loads on all 95 pages — cursor duck, confetti button, silly cursors, etc. (5) share loop encoding the shelf into the URL hash (`store/?s=<base64>`) for a read-only "look what I bought" view — no server, forgeable by design (owner: "just for fun, not concerned about security"). ⚠ **Gag items that render over full-bleed canvas toys MUST be `pointer-events:none` and stay clear of the interaction zone** — this is the exact bug class that hit Nova Coil (tip-jar badge blocking shots) and Dot Loop; a cursor-follower on a drag toy is the same trap. Also: honor `prefers-reduced-motion`, use safe-area insets, and keep the store UI dark-launched behind the same `opt_tickets_ui` flag until owner opens it up. Going live = flip the flag default + add a hub nav link + cache-bust. See Phase 1 in `assets/tickets.js` for the ledger shape and earn API (`window.OPT_TICKETS`).
---
### Ticket system Phase 1 — real-play testing + economy tuning pass

**Why it matters:** The Phase 1 earn rates (active-time accrual + score-improvement awards) were set from first principles, not from real play. Across 87 toys with wildly different session lengths and score scales, unverified rates mean the ticket numbers are arbitrary — and item prices in Phase 2 depend on them being sane.

**When to revisit:** After the owner has played a spread of toys with `?tickets=1` enabled — ideally a few ambient/visual toys (time-only earning) and a few scoring arcade toys (Pool, Nova Coil, Minesweeper, Spelling Blocks, Slice It).
...
---
### New toy: Shinobi-style ninja star throwing game

**Status:** done

**Why it matters:** Owner idea (2026-07-21). The site has arcade score-chases but nothing with the crisp throw-and-hit feel of a shuriken game, and "ninja star" is a strong evergreen search term. Highly juiceable — throwing weapons are all about the impact feedback, which is exactly where this repo's polish bar shines.

**When to revisit:** Next time we're picking new toys to build (owner selects via AskUserQuestion). Pairs well with the retro double-header (Downland / Clowns & Balloons) as an arcade round.

**Notes:** BUILT 2026-07-21 as **No. 088 Shuriken Night** (`toys/shuriken-night/`) — owner chose first-person, moonlit castle courtyard, BOTH camera stances selectable at start, and all four skill layers (deflect / charged fan / ninja magic / focus). Original notes follow. Owner reference is **Shinobi** (Sega, 1987) — the shuriken-throwing side-scrolling ninja action game. **Scope decision needed at build time:** the full Shinobi run-and-gun is too big for one sitting, so pick ONE tight mechanic. Two strong candidates to offer the owner: (a) **throw-and-hit score-chase** — enemies/targets advance in waves from the sides and depth, you aim and throw shuriken with tap/drag (drag = angle + power, arc physics, ricochets), scoring on headshots/multi-kills with a combo chain; or (b) **precision-target range** — a stylized dojo target gallery with moving/hiding targets, bullseye rings, a shot-count budget, and a per-round accuracy grade. Either way: one-sitting arcade, best score in `localStorage`, instant restart. **Design bar:** the star itself must render as a real object (spinning steel with specular flash, motion blur, a trail), impacts should THWACK into wood with splinters and a stuck-star that stays in the board, silhouetted ninja aesthetic with strong lighting — moonlit rooftop or lantern-lit dojo, not flat shapes. Slow-mo on the final kill of a wave. **Audio per the quality bar:** layered throw whoosh (filtered noise sweep), a real wood-thunk impact (transient + resonant woody body + tail) vs. a metallic ting on stone/blade-block, pitch-varied so rapid throws never machine-gun. Legal: original name + original art, mechanics only — no Sega/Shinobi wordmark, sprites, or music. Real rendered card + OG; full add-a-toy pipeline.
---
### New toy: Skee Ball

**Why it matters:** Owner idea (2026-07-21). The single most thematically perfect toy on the list once the **ticket system** exists — skee ball IS the arcade-ticket machine, so it can be the flagship earner and the emotional anchor for the whole store. Also a genuinely great one-sitting score-chase: pure feel, instant restart, no rules to explain.

**When to revisit:** Strong candidate for the next build round, and ideally BEFORE the ticket store (Phase 2) opens to everyone — a real skee ball lane makes the store's premise land instantly.

**Notes:** Classic alley: roll a ball up a lane, it hits the ramp, and lands in concentric scoring rings (10/20/30/40/50, plus the 100 corner holes). Nine balls per game, total score, best in `localStorage`. **Input (LOCKED 2026-07-23):** **swipe-and-release** — one continuous motion, no separate aim step. The swipe's **release velocity** sets roll speed + how hard it vaults off the ramp; the swipe's **direction** sets lateral aim (straight up = center, angled = toward the corners). Mouse is identical — track pointer movement, release on mouse-up. Implementation: sample pointer positions with timestamps and compute velocity from the **last ~80–100ms** (the final flick, not the whole-drag average); minimum-swipe dead-zone so an accidental tap doesn't throw; a live ghost/trail during the swipe for feedback (no separate aim UI); generous tuning so a casual straight swipe still lands a 30–40 while corner-100s demand deliberate angle + power. Physics: ball rolls up the lane, launches off the ramp lip, arcs, and lands — a simple 2D-plus-height sim (x lateral, y up-lane, z height) is enough; no full 3D needed. **Design bar:** render it as the real object — pseudo-3D perspective lane in varnished blond wood with visible grain, brass-ringed target holes, a scuffed dark ball with a specular highlight and a real shadow that separates from the ball as it leaves the ramp, netting/cage sides, warm arcade lighting with a slight vignette. Animated scoreboard with flip/odometer digits (the repo already has odometer craft in Meeting Cost Meter). Ticket-spitter animation on a good round is the perfect flourish. **Audio per the quality bar:** wooden roll rumble that pitches with speed, the hollow *thock* of the ball dropping through a ring, brass rim rattle on a near-miss, and a rising chime ladder for consecutive 40/50s. **⚠ Ticket tie-in:** award via `window.OPT_TICKETS.award()` — the opt-in hook in `assets/tickets.js` — so skee ball pays out real tickets on top of the automatic score detection. Real rendered card + OG; full add-a-toy pipeline.
---
